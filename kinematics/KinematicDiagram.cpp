...
namespace kinematics {
	...
	void KinematicDiagram::removeJoint(boost::shared_ptr<Joint> joint) {
		for (int i = 0; i < joint->links.size(); ++i) {
			for (int j = 0; j < joint->links[i]->joints.size(); ++j) {
				if (joint->links[i]->joints[j] == joint) {
					joint->links[i]->joints.erase(joint->links[i]->joints.begin() + j);
					joint->links[i]->original_shape.remove(joint->id);
					break;
				}
			}
		}
		joints.remove(joint->id);
	}
	...
	boost::shared_ptr<Link> KinematicDiagram::addLink(bool driver, boost::shared_ptr<Joint> joint1, boost::shared_ptr<Joint> joint2) {
		...
		setJointToLink(joint1, link);
		setJointToLink(joint2, link);
